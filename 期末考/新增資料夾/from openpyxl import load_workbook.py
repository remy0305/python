...
wb = load_workbook('51215105.xlsx')
ws = wb.active

TotalTradingVolume = 0
TotalTransactionVolume = 0

for row in range(2, 239):
    cell_value = ws['E' + str(row)].value
    if cell_value is not None:
        TotalTradingVolume += cell_value

        transaction_volume = cell_value * ws['F' + str(row)].value
...
workbook = excel.Workbooks.Open(os.getcwd() + '/' + '51215105.xlsx')
...
pdf_path = os.getcwd() + '/' + '51215105.pdf'
